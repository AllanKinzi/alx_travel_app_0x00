    ...
    def clean(self):
        ...
        if self.guests > self.property.max_guests:
            raise ValidationError(
                f'Number of guests ({self.guests}) exceeds maximum allowed ({self.property.max_guests}).'
            )
...
class Review(models.Model):
    ...
    def __str__(self):
        return f"Review by {self.user.username} for {self.property.title} - {self.rating}/5"